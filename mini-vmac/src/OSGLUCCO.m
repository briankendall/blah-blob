...
static void displayCriticalErrorAndExit(NSString *message)
{
    NSLog(@"%@", message);
...
LOCALVAR NSString *MyResourcePath = nil;
LOCALVAR NSString *MyAppDataPath = nil;
...
LOCALFUNC blnr Sony_Insert2(char *s)
{
    ...
    if (MyAppDataPath && FindNamedChildFilePath(MyAppDataPath, s, &sPath)) {
        return Sony_Insert1(sPath, trueblnr);
    }
...
LOCALFUNC blnr LoadInitialImages(void)
{
	if (! AnyDiskInserted()) {
        ...
        if (v != trueblnr) {
            displayCriticalErrorAndExit(@"Could not load Blah Blob game disk image");
        }
        
        v = Sony_Insert2("BlahBlobData.dsk");
        
        if (v != trueblnr) {
            NSLog(@"Could not load Blah Blob data disk image");
        }
...
LOCALFUNC tMacErr LoadMacRomFromResourcesDir(void)
{
    return LoadMacRomFrom(MyResourcePath);
}
...
LOCALFUNC blnr LoadMacRom(void)
{
    ...
    if (mnvm_fnfErr == (err = LoadMacRomFromResourcesDir()))
...
LOCALPROC MyDrawWithOpenGL(ui4r top, ui4r left, ui4r bottom, ui4r right)
{
	if (nil == MyNSOpnGLCntxt) {
        ...
        //glRasterPos2i(0, vMacScreenHeight * (UseMagnify ? MyWindowScale : 1));
...
#endif
		{
            // Improvement made for Blah Blob:
...
// Blah Blob specific initialization:

static int readAppVersion(NSString *versionPath)
{
    NSError *error = nil;
    ...
    NSString *fileContents = [NSString stringWithContentsOfFile:versionPath
                                                       encoding:NSUTF8StringEncoding
                                                          error:&error];
    if (fileContents == nil) {
        displayCriticalErrorAndExit([NSString stringWithFormat:@"Error: Could not read app version. Error: %@", error]);
        return 0;
    }
    
    int result = [fileContents intValue];
    
    if (result == 0) {
        NSLog(@"Error: App version contained invalid value.");
    }
    
    return result;
}

static void maybeCopyDefaultDiskImage(NSString *initDefaultDiskPath, NSString *defaultDiskPath, NSString *versionPath)
{
    NSError *error = nil;

    if ([[NSFileManager defaultManager] fileExistsAtPath:defaultDiskPath]) {
        int installedVersion = readAppVersion(versionPath);
        
        if (installedVersion >= AppVersion) {
            NSLog(@"Version of installed disk image is same or newer than current. No need to copy!");
            return;
        }
        
        if (![[NSFileManager defaultManager] removeItemAtPath:defaultDiskPath error:&error]) {
            displayCriticalErrorAndExit([NSString stringWithFormat:@"Error: could not remove old disk image in Application Support directory. Error: %@", error]);
            return;
        }
    }
    
    if (![[NSFileManager defaultManager] copyItemAtPath:initDefaultDiskPath toPath:defaultDiskPath error:&error]) {
        displayCriticalErrorAndExit([NSString stringWithFormat:@"Error: could not copy initial disk image to Application Support directory. Error: %@", error]);
        return;
    }
    
    NSLog(@"Copied initial disk image to Application Support directory");
    
    NSString *versionString = [NSString stringWithFormat:@"%d", AppVersion];
    
    BOOL success = [versionString writeToFile:versionPath
                                   atomically:YES
                                     encoding:NSUTF8StringEncoding
                                        error:&error];
    
    if (!success) {
        displayCriticalErrorAndExit([NSString stringWithFormat:@"Error: Could not write app vesion to file. Error: %@", error]);
    }
}

LOCALFUNC blnr setupAppDataDirectory(void)
{
    ...
    NSString *prospectivePath = [[paths firstObject] stringByAppendingPathComponent:@"BlahBlob"];
    NSError *error = nil;

    if (![[NSFileManager defaultManager] createDirectoryAtURL:[NSURL fileURLWithPath:prospectivePath] withIntermediateDirectories:YES attributes:nil error:&error]) {
        displayCriticalErrorAndExit([NSString stringWithFormat:@"Failed to create application support directory. Error: %@", error]);
    }
    
    MyAppDataPath = [prospectivePath retain];
    
    NSString *initDataDiskPath = [MyResourcePath stringByAppendingPathComponent:@"BlahBlobData.dsk"];
    NSString *dataDiskPath = [MyAppDataPath stringByAppendingPathComponent:@"BlahBlobData.dsk"];
    
    if (![[NSFileManager defaultManager] fileExistsAtPath:dataDiskPath]) {
        if (![[NSFileManager defaultManager] copyItemAtPath:initDataDiskPath toPath:dataDiskPath error:&error]) {
            displayCriticalErrorAndExit([NSString stringWithFormat:@"Error: could not copy data disk image to Application Support directory. Error: %@", error]);
            return falseblnr;
        }
        
        NSLog(@"Copied initial data disk image to Application Support directory");
    }
    
    NSString *initDefaultDiskPath = [MyResourcePath stringByAppendingPathComponent:@"BlahBlob.dsk"];
    NSString *defaultDiskPath = [MyAppDataPath stringByAppendingPathComponent:@"BlahBlob.dsk"];
    NSString *versionPath = [MyAppDataPath stringByAppendingPathComponent:@"version.dat"];
    maybeCopyDefaultDiskImage(initDefaultDiskPath, defaultDiskPath, versionPath);
...
LOCALFUNC blnr setupWorkingDirectory(void)
{
    ...
    MyResourcePath = [myBundle resourcePath];
    [MyResourcePath retain];
...
LOCALPROC UnInitCocoaStuff(void)
{
    ...
    if (nil != MyResourcePath) {
        [MyResourcePath release];
    }
    if (nil != MyAppDataPath) {
        [MyAppDataPath release];
    }
}
